...
def test_relative_path():
    captured_dot_path = subprocess.run(
        ["pyls", "."], capture_output=True, text=True
    ).stdout
    expected_dot_path = "LICENSE README.md ast go.mod lexer main.go parser token\n"

    captured_parser_path = subprocess.run(
        ["pyls", "./parser"], capture_output=True, text=True
    ).stdout
    expected_parser_path = "parser_test.go parser.go go.mod\n"

    captured_trailing_slash = subprocess.run(
        ["pyls", "./parser/"], capture_output=True, text=True
    ).stdout
...
def test_path_as_file_name():
    captured_file_name = subprocess.run(
        ["pyls", "./parser/parser.go"], capture_output=True, text=True
    ).stdout
