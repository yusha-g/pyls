...
def test_filter_dir():
    captured_result = subprocess.run(["pyls","--filter=dir"], capture_output=True, text=True).stdout
    expected_result = "ast lexer parser token\n"
    assert captured_result == expected_result

def test_filter_dir_lost_reverse_time():
    captured_result = subprocess.run(["pyls","-lrt","--filter=dir"], capture_output=True, text=True).stdout
    expected_result = (
...
def test_filter_file():
    captured_result = subprocess.run(["pyls","--filter=file"], capture_output=True, text=True).stdout
    expected_result = "LICENSE README.md go.mod main.go\n"
    assert captured_result == expected_result

def test_filter_file_lost_reverse_time():
    captured_result = subprocess.run(["pyls","-lrt","--filter=file"], capture_output=True, text=True).stdout
    expected_result = (
